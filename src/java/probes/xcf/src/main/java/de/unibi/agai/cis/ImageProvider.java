...
import de.unibi.agai.image.log.StreamLogger;
...
/**
 * Generic description of an image: width, height, pixel organization, and so on
 * plus helper functions to convert it to various representations. This is
 * basically used to give the user the more-or-less raw image data and provide a
 * means to get it in the desired container.
 *
...
public class ImageProvider {
    ...
    public IVideoPicture createPicture(IVideoPicture pic) throws IOException {
        switch (ptype) {
            case GRAY8:
     ...
     * possible. Note: The array /will/ contain new buffers after this method
     * returns, even if the the array itself is still the same.
     */
    ...
    protected IVideoPicture configurePic(IVideoPicture pic, IPixelFormat.Type type) {
        ...
        if (pic == null || pic.getPixelType() != type
                || pic.getWidth() != width || pic.getHeight() != height) // NOTE: we could do this much more efficiently (by passing a
    ...
    private IVideoPicture createPictureFromPlanarRGB(IVideoPicture pic) throws IOException {
        BufferedImage bufImg = new BufferedImage(width, height, BufferedImage.TYPE_BYTE_BINARY);
        WritableRaster wr = bufImg.getRaster();
        wr.setDataElements(0, 0, width, height, pixelData);
...
//            buf.put(pixelData, i, 1);
//            buf.put(pixelData, (pixelData.length / 3) + i, 1);
//            buf.put(pixelData, (pixelData.length / 3) * 2 + i, 1);
//        }
//        return pic;
